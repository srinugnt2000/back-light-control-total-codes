...
echo 250 > /sys/class/backlight/pwm-backlight/brightness
(100%)
...
//	printf("\nIn child process\n");
  ...
  while(1)
	...
	system("echo 1 > /sys/class/backlight/pwm-backlight/brightness");
	}		
...
int sub_chld_process(void *arg)
{
	
	struct input_event sevth;
	FILE *skth = fopen("/dev/input/event1", "r");	
	sleep(5);
	while (1)
	{
		while ((fread(&sevth, sizeof(sevth), 1, skth) == 1) )
		{
		    if (lcd_off_flag == 0)
		       {			
			lcd_off_flag = 1;
			bug = 1;
			system("stop-script");
			system("echo 250 > /sys/class/backlight/pwm-backlight/brightness");
			kb_flag=0;
			th_flag=0;
		       }
		}
...
void *sub_child_stack;
	...
	if(child_stack==NULL){
		...
		sub_child_stack= malloc(STACK_SIZE);
	if(child_stack==NULL){
		perror("\nError creating child stack\n");
		exit(1);
		}
...
/****************************creating sub child process*****************************/
 clone( sub_chld_process, sub_child_stack+STACK_SIZE, CLONE_VM|CLONE_FILES, NULL);
/****************************creating sub child process ****************************/	
